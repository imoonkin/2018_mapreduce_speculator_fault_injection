...
public class DefaultSpeculator extends AbstractService implements
  ...
  private static final long MAP_SPE = Long.MIN_VALUE + 6;
...
//  those jobs that need them.
  ...
  private long speculationValue(TaskId taskID, long now) {
    ...
    for (TaskAttempt taskAttempt : attempts.values()) {
      if (taskAttempt.getState() == TaskAttemptState.RUNNING
        ...
        if (task.getType()==TaskType.MAP){
            System.out.println("maybe_foundMAP_SPE");
            if(now-taskAttemptStartTime>60000 || clock.getTime()-taskAttemptStartTime>60000){
                System.out.println("surely_foundMAP_SPE");
                return MAP_SPE;
            }
        }
        if (progress>fastest){
    ...
    System.out.println("thisistag");
    System.out.println(fastest);
    System.out.println(now);
    return (long)(100 - fastest);
  ...
  private int maybeScheduleASpeculation(TaskType type) {
    ...
    for (ConcurrentMap.Entry<JobId, AtomicInteger> jobEntry : containerNeeds.entrySet()) {
      ...
      int numberAllowedSpeculativeTasks=0;
      ...
      for (Map.Entry<TaskId, Task> taskEntry : tasks.entrySet()) {
        ...
        if (mySpeculationValue == MAP_SPE){
            numberAllowedSpeculativeTasks
            = (int) Math.max(numberAllowedSpeculativeTasks,
                proportionRunningTasksSpeculatable * numberRunningTasks);
            System.out.println("maybe_usedMAP_SPE");
            if (numberAllowedSpeculativeTasks+2 > numberSpeculationsAlready){
                addSpeculativeAttempt(taskEntry.getKey());
                System.out.println("surely_usedMAP_SPE");
                ++successes;
                return successes;
            }
        }
        count_task+=1;
        totalpercentage+=mySpeculationValue;
      ...
      if (bestTaskID != null
        ...
        double avgpercentage=(double)totalpercentage/count_task;
        if (bestSpeculationValue-35>avgpercentage){
